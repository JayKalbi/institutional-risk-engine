...
app = Flask(__name__, template_folder=os.path.join(base_dir, 'flask-app', 'templates'), static_folder=os.path.join(base_dir, 'flask-app', 'static'))
...
def narrative():
    """H100 Local vLLM Inference with fallback to Groq or RAG Dynamic Engine."""
    data = request.json or {}
...
"""
    ...
    # 1. First Attempt Local vLLM Server on H100 (Port 8000)
    ...
    try:
        vllm_res = requests.post(vllm_url, json=vllm_payload, timeout=5)
        if vllm_res.status_code == 200:
            out = vllm_res.json()
            if "choices" in out and len(out["choices"]) > 0:
                print("[H100 ROUTER] Successfully generated via Local H100 vLLM Server!", flush=True)
                return jsonify({"narrative": out["choices"][0]["message"]["content"].strip(), "source": "H100 Local GPU Cluster (vLLM)"})
    except Exception:
        pass

    # 2. Fallback to Groq API if Key Provided
    if api_key:
        ...
        for model_name in ["llama-3.3-70b-versatile", "llama3-70b-8192", "mixtral-8x7b-32768"]:
            ...
            try:
                ...
                if response.status_code == 200:
                    out = response.json()
                    if "choices" in out and len(out["choices"]) > 0:
                        return jsonify({"narrative": out["choices"][0]["message"]["content"].strip(), "source": f"Groq AI ({model_name})"})
            except Exception:
                pass

    # 3. Dynamic Fallback Engine
